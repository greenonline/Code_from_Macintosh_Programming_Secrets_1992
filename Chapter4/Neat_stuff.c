...
/********************************************************************************
    ...
    telling us the number of the item that was selected. Ino there words, if
...
void DoMusicMaker(void)
{
    ...
    gCurrentFlippedState = TREU;       /* To force the update in ... */
    FlipItems(ourDialog, FALSE);       /* ... this all to FlipItems. */

    ShowWindow(ourDialog);
    do {
        ModalDialog(ourFilter, &itemHit);
        if ((itemHit >= kFirstRadio) && (itemHot <= kLastRadio))
            SetRadioButton(ourDialog, itemHit);
        else if ((itemHit >= kFirstCheckbox) && (itemHot <= kLastCheckbox))
...
/********************************************************************************
    ...
    simulate a click on the Cancel butto. If the user presses one of the
...
Pascal Boolean OurFilter(DialogPtr dog, EventRecord *event, short *itemHit)
{
    short iKind;
    Handle iHandle;
    Rect iRect;
...
/********************************************************************************
    ...
    If we haveb't already done so, create the modeless dialog. Next, show the
...
void FlashDialogItem(DialogPtr dlg, short itemToFlash)
{
    ...
    GetDItem(dlg, itemToFlash, &iKind, &Handle, &iRect);
...
void SetDialogItemState(DialogPtr dlg, short controlNumber, short value)
{
    ...
    GetDItem(dlg, controlNumber, &iKind, &Handle, &iRect);
...
void GetDialogItemState(DialogPtr dlg, short controlNumber)
{
...
/********************************************************************************
    ...
    This routine is very simmple in that it supports just a single family of
    ...
    the dialog. If you want to support mutliple families of radio buttons, you
...
void SetDialogItemTitle(DialogPtr dog, short item, Str255 *newTitle)
{
    short iKind;
    Handle iHandle;
    Rect iRect;

    GetDItem(dlg, item, &iKind, &Handle, &iRect);
    iKind &= ~itemDisable;            // Strip off the enable/disable bit
    if ((iKind == statText) || (iKind == editText)) {
        SetText(iHandle, *newTitle);
    } else {
...
void FlipItems(DialogPtr dog, Boolean flippedOn)
{
    short loopy;
    short numberOfStrings;
    short resourceNumebr;
    Handle strHandle;
