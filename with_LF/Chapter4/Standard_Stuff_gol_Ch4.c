...
//Boolean gPeriodicTask;   // Defined in NeatStuff.c
...
void AdjustMenus(){
  ...
  window = FrontWindow();
  //menu = GetMHandle(mFile);  // gol - commented out and put below
  ...
  menu = GetMHandle(mFile);  // gol - moved from above
  if (IsDAWindow(window))    // This does not enable close for modeless dialog
  //if (window != NIL)       // This enables close for the modeless dialog
